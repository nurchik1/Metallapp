...
function dialogAlert() {
 ...
 function fillLoginData(){
    document.getElementById("userSigninLogin").value = userInfo.getItem('email');
    document.getElementById("userSigninPassword").value = userInfo.getItem('password');
 }
$('form').submit(function(){
    var emailValue = document.getElementById("userSigninLogin").value;
    var passwordValue = document.getElementById("userSigninPassword").value;
    $.ajax({
        type: 'POST',
        url: 'http://ahbasys.com/finans/api/login',
        data: jQuery.param({ email: emailValue, password : passwordValue}) ,
        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
        success: function (resp) {
            userInfo.setItem('token', resp.token);
            userInfo.setItem('region_id', resp.user.region_id);
            userInfo.setItem('role_id', resp.user.role_id);
            userInfo.setItem('email', resp.user.email);
            userInfo.setItem('password', resp.user.password);
            userInfo.setItem('user_id', resp.user.id);
            if(resp.user.role_id == 2){
                window.location.href = "manager.html";
            }
            else if(resp.user.role_id == 3){
