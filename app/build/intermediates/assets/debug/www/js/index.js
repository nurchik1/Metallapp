...
function getProduct() {
        ...
        var data = "";
                for (i = 0; i < productcat.length; i++) {
				    ...
				    for (j = 0; j < productcat[i].products.length; j++) {
	                    ...
	                    data += '</td>';
	                    data += '<td>';
	                    data += '<input id = "product_quantity" type="number" class="form-control input-smally check-null-qty" name="qty[]" min="1"  onkeyup="getValues()" onchange="getValues()">';
	                    data += '</td>';
  ...
  $('form').submit(function(){
    saleInfo = window.localStorage;
    var price = $('input[name="price[]"]').map(function(){ return this.value; }).get();
    var qty = $('input[name="qty[]"]').map(function(){ return this.value; }).get();
    var product_id = $('input[name="product_id[]"]').map(function(){ return this.value; }).get();
    logged_user = saleInfo.getItem('user_id');
    region_id = saleInfo.getItem('region_id');
    $.ajax({
        type: 'POST',
        url: 'http://ahbasys.com/finans/api/v1/salegroup?token='+saleInfo.getItem('token'),
        data: {
            'price[]': price,
            'qty[]': qty,
        ...
        success: function (resp) {
            saleInfo.setItem('unique_id', resp.unique_id);
            saleInfo.setItem('sale_group_id', resp.id);
            window.location.href = "sale-confirm.html";
        },
        error: function (e) {
            alert("Ошибка");
        }
